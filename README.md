...
npx supabase start
```
*Note: Copy the API URL and anon key printed by this command into your `.env.local` file. The application will not start without them.*
...
*(Never expose the `service_role` key to this Vite application. The bootstrap function permanently refuses to run once an agency has an active principal).*
