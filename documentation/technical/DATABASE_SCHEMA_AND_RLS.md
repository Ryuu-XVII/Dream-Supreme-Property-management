...
**STABLE-function fix (`20260825130917_fix_rate_limited_functions_marked_stable.sql`)**: `check_rate_limit`'s `INSERT` gives any function that calls it a side effect, but `get_current_transfer_duty_brackets`, `get_status_request`, and `validate_user_invitation` were still declared `STABLE`. PostgREST opens a read-only transaction for `STABLE`/`IMMUTABLE` functions, so every real call to these three via the REST API failed outright with `HTTP 405 — cannot execute INSERT in a read-only transaction` (Postgres error `25006`) from the moment `20260817000000_rate_limiting.sql` landed — unconditionally, not just under load. This took out the public Transfer Duty Calculator, the public deal-status lookup page, and invited-agent registration's invite-token validation. Direct SQL/the Postgres wire protocol never exercises PostgREST's transaction-mode logic, which is why it went unnoticed until tested over the real REST API. All three are now plain (volatile) functions; bodies are unchanged.
...
## 32. The Notification Matrix Never Actually Gated Anything (`20260825134351`)

The admin "Notification Matrix" screen (`src/routes/admin/notifications.tsx`) reads and writes `public.notification_preference` correctly, but no code path that created a `public.notification` row or an `email_queue` row ever consulted that table. Every email/in-app toggle, recipient-role selection, and condition JSON was a pure no-op — confirmed by grepping every migration for a `from`/`join` on `notification_preference` from anywhere other than the frontend query: zero hits. On top of that, 4 of the matrix's 11 event types had no trigger path at all (Condition failed, Deal stale, Commission issued, New lead), and every working path only ever wrote in-app rows, never `email_queue`, so the Email column did nothing anywhere.

**Fix:** added `public.dispatch_notification(...)`, a single `SECURITY DEFINER` helper every notification-creating path now goes through. It looks up the `(agency_id, event_type)` preference row; with no saved row (an agency that's never opened the matrix, or an event type — like "Deal opened" or "Deal progress note" — that isn't one of the 11 matrix rows) it delivers unrestricted on both channels, preserving prior behavior. Otherwise it honors `email_enabled`/`in_app_enabled`, filters by `recipient_roles` (an `admin_agent` recipient satisfies either an `'admin'` or `'agent'` role entry, since that account is both), and — the one condition shape the UI's own placeholder documents — skips delivery if `condition_config` has a `min_price` key and the caller's `p_price_cents` argument is both non-null and below it (a null price, i.e. an event with no natural price context, is never gated by a price condition). An optional `p_dedupe_key` reuses `check_rate_limit` (1 attempt / 1 day) as a generic "already notified this recipient about this exact thing recently" check, replacing the bespoke `not exists` bookkeeping queries `run_daily_sweeps()` used to hand-roll per notification type.

Rewired every existing notification-creating path to call it instead of inserting directly: `notify_agency_admins()` (deal registered/cancelled/stage advanced), `run_daily_sweeps()` (condition due, mandate expiring, FFC expiring), `submit_conveyancer_status()`. Added it to the four dead types: `set_condition_status()` now notifies on `p_status = 'failed'`; `calculate_deal_commission()` notifies each internal `deal_participant` with their allocation once commission is calculated (price-gated by `v_gross`, so a `min_price` condition on "Commission issued" filters by gross commission size); `create_status_request()` now notifies internal recipients when a conveyancer status-request link is generated (the existing "conveyancer responded" notification in `submit_conveyancer_status()` covers the other leg, both tagged `'Conveyancer status request'` since the matrix has one row for both); and a new `AFTER INSERT` trigger on `public.lead` (`notify_new_lead()`) notifies every active agent/admin.

Also added a new "Deal stale" sweep to `run_daily_sweeps()` — a concept that didn't exist anywhere in the schema before this. Defined as an active deal whose row hasn't been touched (any `UPDATE` bumps `updated_at` via the existing `touch_updated_at` trigger, so this is effectively "no stage/status/field movement") in 14+ days, re-notified weekly rather than nightly via the dedupe key's date-bucketed suffix.

**Default-recipient seeding:** without a stored preference row, `dispatch_notification` delivers unrestricted — correct as a fallback, but it meant "Commission issued" would reach agents too until an admin happened to open and save the matrix once. Seeded the same defaults the frontend itself starts new rows with (email + in-app on, `{agent, admin}`, except Commission issued which defaults to `{admin}`) for every existing agency, and added an `AFTER INSERT` trigger on `public.agency` (`seed_default_notification_preferences()`) so future agencies get them too.

**Bug caught by testing, not review:** the first version of the `min_price` condition check read `p_price_cents is null or p_price_cents < threshold` as the *skip* condition — meaning an event with no price context (the intended "not gated" case) was silently treated as failing the gate instead. Caught by directly exercising `dispatch_notification` against a real account with a battery of `select public.dispatch_notification(...)` smoke tests (email on/off, role restriction, `admin_agent` dual-role matching, dedupe, and all three `min_price` cases) before trusting the migration, not by re-reading the SQL. Fixed to only skip when `p_price_cents is not null and p_price_cents < threshold`.

**Drift reconciliation (2026-08-29):** production's `dispatch_notification` body had drifted from the migration history — the deployed function had its comments and whitespace trimmed by some out-of-band edit, though its logic was unchanged. `supabase db diff --linked` was used to capture the live definition into a follow-up migration so the two stay in sync; no behavioral change.
