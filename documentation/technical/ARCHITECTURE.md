...
- **Storage Fallback & Signed URLs**: The `r2-storage` Edge Function requires the `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, and `R2_BUCKET_NAME` Supabase secrets (set via `supabase secrets set`, never in `.env`/`VITE_`-prefixed variables). If those secrets are unset or the function is unreachable, `src/lib/storage.ts` transparently falls back to Supabase Storage (`mandate-documents`) rather than failing uploads outright — the **Admin > System Settings > Document Storage** card reports which backend is actually active. All files remain strictly private, accessed via presigned URLs (300s for R2 downloads via `getR2FileUrl`, or Supabase's own signed URLs in fallback mode).
