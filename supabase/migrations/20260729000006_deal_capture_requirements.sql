-- Research-backed capture fields needed to hand a South African residential sale
-- from the property practitioner to the conveyancer without re-keying the deal.

alter type public.document_category add value if not exists 'property_disclosure';
alter type public.document_category add value if not exists 'seller_fica';
alter type public.document_category add value if not exists 'purchaser_fica';
alter type public.document_category add value if not exists 'entity_authority';
alter type public.document_category add value if not exists 'connected_party_valuations';
alter type public.document_category add value if not exists 'section_35a';
alter type public.document_category add value if not exists 'vat_registration';

alter table public.property
  add column if not exists legal_description text,
  add column if not exists deeds_office text,
  add column if not exists property_use text,
  add column if not exists is_improved boolean not null default true,
  add column if not exists seller_acquired_on date,
  add column if not exists seller_original_purchase_price_cents bigint;

alter table public.party
  add column if not exists tax_number text,
  add column if not exists date_of_birth date,
  add column if not exists nationality text,
  add column if not exists is_sa_resident boolean not null default true,
  add column if not exists passport_number text,
  add column if not exists passport_country text,
  add column if not exists representative_name text,
  add column if not exists representative_capacity text,
  add column if not exists beneficial_owner_details text,
  ...
  add column if not exists is_prominent_person boolean not null default false;

alter table public.deal
  add column if not exists effective_date date,
  add column if not exists offer_expires_on date,
  add column if not exists deposit_cents bigint not null default 0,
  add column if not exists deposit_due_on date,
  add column if not exists deposit_holder text,
  add column if not exists balance_payment_method text,
  add column if not exists occupational_rent_cents bigint not null default 0,
  add column if not exists sale_method text,
  add column if not exists transfer_share_percent numeric(7,4) not null default 100,
  add column if not exists parties_connected boolean not null default false,
  add column if not exists seller_is_non_resident boolean not null default false,
  add column if not exists property_disclosure_completed boolean not null default false,
  add column if not exists disclosure_defects text,
  add column if not exists fixtures_included text,
  add column if not exists fixtures_excluded text,
  add column if not exists special_conditions text,
  add column if not exists vat_inclusive boolean not null default true;

alter table public.deal_party
  add column if not exists share_percent numeric(7,4) not null default 100;

do $$ begin
  alter table public.deal_party add constraint deal_party_share_valid
    check (share_percent > 0 and share_percent <= 100) not valid;
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.deal add constraint deal_transfer_share_valid
    check (transfer_share_percent > 0 and transfer_share_percent <= 100);
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.deal add constraint deal_money_values_non_negative
    check (sale_price_cents > 0 and deposit_cents >= 0 and occupational_rent_cents >= 0) not valid;
exception when duplicate_object then null; end $$;
