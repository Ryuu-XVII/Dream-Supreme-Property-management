-- Create the cascading version WITHOUT auth checks temporarily
...
declare
  ...
  v_invalid_ffc text;
begin
  -- TEMPORARILY REMOVED FOR MIGRATION:
  ...
  insert into public.commission_calculation (deal_id, rule_set_id, calculated_by, gross_cents, vat_cents, net_cents, distributable_pool_cents, office_share_cents, agent_pool_cents, input_snapshot_json, status)
  values (
    p_deal_id, v_rule.id, public.get_current_user_account_id(),
    v_gross, v_vat, v_net, v_pool, v_office, v_agent_pool,
    jsonb_build_object(
...
declare
  d record;
begin
  for d in select id from public.deal loop
    perform public.calculate_deal_commission(d.id);
  end loop;
end;
$$;

-- Restore the function WITH auth checks
create or replace function public.calculate_deal_commission(
  p_deal_id uuid,
  p_rule_set_id uuid default null
) returns uuid
