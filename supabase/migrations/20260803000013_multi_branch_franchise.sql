-- Add franchise fee percentage to branch
alter table public.branch
add column if not exists franchise_fee_pct numeric(5,2) not null default 0;

-- Add franchise_fee_cents to commission_calculation
alter table public.commission_calculation
add column if not exists franchise_fee_cents bigint not null default 0;

-- Update the commission calculation engine to deduct franchise fee before agent allocations
