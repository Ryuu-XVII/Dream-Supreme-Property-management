-- 20260818000008_admin_only_commission.sql intended to close a NULL-role
-- authorization bypass in calculate_deal_commission (a caller whose
-- get_current_role() is NULL -- a suspended agent whose JWT is still valid,
-- or an unaccepted registration -- has `NULL not in (...)` evaluate to NULL,
-- not true, so the guard silently fell through instead of raising).
--
-- But that migration added a third parameter (p_override boolean), and
-- `create or replace function` with a different argument list creates a new
-- overload rather than replacing the existing one. The original two-argument
-- calculate_deal_commission(uuid, uuid) -- the one directly granted to
-- `authenticated` and callable as a PostgREST RPC -- was left behind with
-- its original, unguarded check. This re-declares that exact signature with
-- the same coalesced guard the three-argument overload already has; the rest
-- of the function body is unchanged from what is live today.
