...
export const Route = createFileRoute("/leads")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage leads captured from the bond, transfer, affordability and yield calculators." },
      { property: "og:title", content: "Leads | Dream Supreme Properties" },
      { property: "og:description", content: "Manage leads captured from the bond, transfer, affordability and yield calculators." },
    ],
  }),
  component: LeadsPage,
});
...
function SourceBadge({ source }: { source: Lead["source"] }) {
  return <Badge variant="outline" className={cn(SOURCE_TONE[source])}>{source}</Badge>;
}

function StatusBadge({ status }: { status: Lead["status"] }) {
  return <Badge variant="outline" className={cn(STATUS_TONE[status])}>{status}</Badge>;
}
...
function parsePayload(payload: string) {
  ...
  const parts = payload.split(/[,]|\bat\b|\bover\b|\bwith\b|\bfor\b/i).map((s) => s.trim()).filter(Boolean);
  return parts;
}
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-md">
        <SheetHeader>
          ...
          <SheetDescription>Lead captured {dateFmt(draft.createdAt)} via {draft.source} calculator</SheetDescription>
        </SheetHeader>

        <div className="mt-6 space-y-6">
          <div>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Contact info</h3>
            <div className="space-y-1.5 rounded-lg border border-border bg-muted/30 p-3 text-sm">
              <p className="flex justify-between gap-2"><span className="text-muted-foreground">Email</span><span className="min-w-0 truncate">{draft.email}</span></p>
              <p className="flex justify-between gap-2"><span className="text-muted-foreground">Mobile</span><span className="money">{draft.mobile}</span></p>
              <p className="flex justify-between gap-2"><span className="text-muted-foreground">Source</span><SourceBadge source={draft.source} /></p>
            </div>
          </div>

          <div>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Calculator payload</h3>
            <div className="space-y-1.5 rounded-lg border border-border bg-muted/30 p-3 text-sm">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setDraft({ ...draft, assignedTo: v === "unassigned" ? undefined : v })}
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="unassigned">Unassigned</SelectItem>
                {users.filter((u) => u.role === "Agent" || u.role === "Candidate").map((u) => (
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              onClick={() => {
                onSave(draft);
                toast.success("Lead updated", { description: `${draft.name}'s record has been saved.` });
                onOpenChange(false);
              }}
            >
              Save changes
...
function LeadsTable({ rows, onOpen, onQuickAssign, onQuickStatus }: {
  rows: Lead[];
  onOpen: (l: Lead) => void;
  ...
  const [visibleCols, setVisibleCols] = useState<Record<SortKey, boolean>>({
    name: true, email: true, mobile: true, source: true, agent: true, status: true, createdAt: true,
  });
  ...
  const sorted = useMemo(() => {
    ...
    withMeta.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "mobile": cmp = a.lead.mobile.localeCompare(b.lead.mobile); break;
        case "source": cmp = a.lead.source.localeCompare(b.lead.source); break;
        case "agent": cmp = a.agent.localeCompare(b.agent); break;
        case "status": cmp = a.lead.status.localeCompare(b.lead.status); break;
        case "createdAt": cmp = new Date(a.lead.createdAt).getTime() - new Date(b.lead.createdAt).getTime(); break;
      }
      return cmp * sortDir;
    });
    return withMeta;
  ...
  if (rows.length === 0) {
    return <EmptyState icon={Users} title="No leads match your filters" message="Try widening your filters or resetting them to see more leads." />;
  }
          ...
          <TableBody>
            {pageRows.map(({ lead }) => (
              <TableRow key={lead.id} className="cursor-pointer" onClick={() => onOpen(lead)}>
                {visibleCols.name && <TableCell className="max-w-[160px] truncate font-medium">{lead.name}</TableCell>}
                {visibleCols.email && <TableCell className="max-w-[200px] truncate text-muted-foreground">{lead.email}</TableCell>}
                {visibleCols.mobile && <TableCell className="money whitespace-nowrap">{lead.mobile}</TableCell>}
                {visibleCols.source && <TableCell><SourceBadge source={lead.source} /></TableCell>}
                {visibleCols.agent && (
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    ...
                    >
                      <SelectTrigger className="h-8 w-[160px] text-xs">
                        {lead.assignedTo ? <AgentAvatar user={userById(lead.assignedTo)} showName size={5} /> : <SelectValue placeholder="Assign..." />}
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="unassigned">Unassigned</SelectItem>
                        {users.filter((u) => u.role === "Agent" || u.role === "Candidate").map((u) => (
                ...
                {visibleCols.status && (
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Select value={lead.status} onValueChange={(v) => onQuickStatus(lead.id, v as Lead["status"])}>
...
function LeadsPage() {
  ...
  const filtered = useMemo(() => {
    return leads.filter((l) => {
      ...
      if (filters.agent !== "all" && filters.agent !== "unassigned" && l.assignedTo !== filters.agent) return false;
  ...
  const quickAssign = (id: string, agent: string) => {
    setLeads((prev) => prev.map((l) => (l.id === id ? { ...l, assignedTo: agent === "unassigned" ? undefined : agent } : l)));
    toast.success("Lead assigned", { description: agent === "unassigned" ? "Lead is now unassigned." : `Assigned to ${userById(agent).name}.` });
  };
  ...
  return (
    <AppShell title="Leads" description="Leads captured from the public calculators, ready to assign and convert.">
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}>
          <LeadsTable
            rows={filtered}
            onOpen={(l) => { setActiveLead(l); setSheetOpen(true); }}
            onQuickAssign={quickAssign}
            onQuickStatus={quickStatus}
          />
        </motion.div>
      )}
      <LeadSheet lead={activeLead} open={sheetOpen} onOpenChange={setSheetOpen} onSave={updateLead} />
    </AppShell>
  );
}
