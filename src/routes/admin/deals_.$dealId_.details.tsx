      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <GlassCard>
          ...
          <div className="mt-6 space-y-5 text-sm leading-relaxed text-foreground/90">
            <p>
              ...
              {deal.property.beds} bedroom{deal.property.beds === 1 ? "" : "s"},{" "}
              {deal.property.baths} bathroom{deal.property.baths === 1 ? "" : "s"} and{" "}
              {deal.property.garages} garage{deal.property.garages === 1 ? "" : "s"}. The erf
              measures {deal.property.erfSize} m² with a floor size of {deal.property.floorSize} m².
              The property is managed under the {deal.branch} branch.
            </p>

            <p>
              <span className="font-semibold text-foreground">Mandate &amp; Sale.</span> The
              property was listed under a {deal.mandateType} mandate at a listing price of{" "}
              {zar(deal.listingPrice, { decimals: false })}
              {deal.mandateSigned ? `, signed on ${dateFmt(deal.mandateSigned)}` : ""}
              {deal.mandateExpiry ? ` and expiring ${dateFmt(deal.mandateExpiry)}` : ""}. The deal
              is currently at the <span className="font-medium">{deal.stage}</span> stage, with a
              sale price of {zar(deal.salePrice, { decimals: false })}
            ...
            {sellers.length > 0 && (
              <p>
                <span className="font-semibold text-foreground">Seller{sellers.length > 1 ? "s" : ""}.</span>{" "}
                {sellers
                  .map(
                    (s: any) =>
            ...
            {deal.bond?.status && deal.bond.status !== "Not applied" && (
              <p>
                <span className="font-semibold text-foreground">Bond.</span> A bond application
                with status "{deal.bond.status}" was
        ...
        <GlassCard>
          ...
          {documents.isLoading ? (
            ...
            <EmptyState title="No documents" message="No documents have been submitted for this deal." />
          ) : (
