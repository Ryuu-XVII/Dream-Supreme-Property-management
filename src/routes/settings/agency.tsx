...
export const Route = createFileRoute("/settings/agency")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage agency details, branches, conveyancer firms and transfer duty brackets." },
      { property: "og:title", content: "Agency Profile | Dream Supreme Properties" },
      { property: "og:description", content: "Manage agency details, branches, conveyancer firms and transfer duty brackets." },
    ],
  }),
...
function AgencyProfilePage() {
  ...
  const [branchList, setBranchList] = useState<Branch[]>(initialBranches.map((b) => ({ ...b, archived: false })));
  ...
  return (
    <AppShell title="Settings" description="Agency profile, branches, conveyancers and transfer duty configuration.">
      <SettingsTabs />
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <div className="grid gap-5 sm:grid-cols-2">
                  <div className="flex items-center gap-4 sm:col-span-2">
                    ...
                    <div>
                      <Label htmlFor="logo-upload" className="text-xs font-medium">Agency Logo</Label>
                      <Input id="logo-upload" type="file" accept="image/*" onChange={handleLogo} className="mt-1 max-w-xs" />
                    </div>
                  </div>
                  <FormField
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input {...field} className="money" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
            ...
            <TableBody>
              {branchList.map((b) => (
                <TableRow key={b.id} className={b.archived ? "opacity-50" : ""}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={b.archived ? "bg-muted text-muted-foreground" : "border-success/30 bg-success/10 text-success"}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label>Address</Label>
              <Textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={2} className="mt-1" />
            </div>
          </div>
          <DialogFooter>
...
function ConveyancerSection({
  ...
  const [draft, setDraft] = useState({ name: "", contact: "", email: "", tel: "", preferred: false });
  ...
  function startEdit(f: Firm) {
    setEditing(f);
    setDraft({ name: f.name, contact: f.contact, email: f.email, tel: f.tel, preferred: f.preferred });
    setOpen(true);
  }
  function save() {
          ...
          <TableBody>
            {firmList.map((f) => (
              <TableRow key={f.id}>
                ...
                <TableCell>
                  {f.preferred && <Badge variant="outline" className="border-primary/30 bg-primary/10 text-primary">Preferred</Badge>}
                </TableCell>
                <TableCell className="text-right">
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input value={draft.contact} onChange={(e) => setDraft((d) => ({ ...d, contact: e.target.value }))} className="mt-1" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input value={draft.tel} onChange={(e) => setDraft((d) => ({ ...d, tel: e.target.value }))} className="mt-1" />
              </div>
            </div>
            <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
              <Label className="mb-0">Preferred Firm</Label>
              <Switch checked={draft.preferred} onCheckedChange={(v) => setDraft((d) => ({ ...d, preferred: v }))} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
...
function TransferDutySection({
  ...
  function saveBrackets() {
    toast.success("Transfer duty brackets saved", { description: `Effective ${dateFmt(effectiveDate)}` });
  }
  ...
  return (
    <GlassCard>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <span className="inline-flex items-center gap-1"><Percent className="size-3" /> Rate</span>
              </TableHead>
              <TableHead className="text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
              ...
              <TableRow key={b.id}>
                <TableCell>
                  <Input
                    ...
                    value={b.from / 100}
                    onChange={(e) => update(b.id, { from: Math.round(Number(e.target.value) * 100) })}
                  />
                </TableCell>
                <TableCell>
                  <Input
                    ...
                    onChange={(e) =>
                      update(b.id, { to: e.target.value === "" ? null : Math.round(Number(e.target.value) * 100) })
                    }
                  />
                </TableCell>
                <TableCell>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button size="sm" onClick={saveBrackets}>Save Brackets</Button>
      </div>
    </GlassCard>
  );
}
