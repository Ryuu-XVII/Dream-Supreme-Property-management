...
export const Route = createFileRoute("/conveyancer")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Submit a status update on a property transfer for Dream Supreme Properties." },
      { property: "og:title", content: "Conveyancer Status Update | Dream Supreme Properties" },
      { property: "og:description", content: "Submit a status update on a property transfer for Dream Supreme Properties." },
    ],
  }),
...
const deal = dealById(deals.find((d) => d.stage === "Documents & Guarantees")?.id ?? deals[0].id);
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-8 sm:px-8">
        ...
        <p className="mb-6 text-sm text-muted-foreground">
          Please confirm progress on this transfer. No login required — this link is unique to this deal.
        </p>
        ...
        {view === "form" && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <GlassCard>
              <dl className="space-y-3 text-sm">
                ...
                <div className="flex items-start justify-between gap-2">
                  <dt className="flex items-center gap-1 text-muted-foreground"><MapPin className="size-3.5" /> Property</dt>
                  <dd className="max-w-[65%] text-right font-medium">{property.address}, {property.suburb}</dd>
                </div>
                <div className="flex items-center justify-between gap-2">
                  <dt className="text-muted-foreground">Current stage</dt>
                  <dd><StageBadge stage={deal.stage} large /></dd>
                </div>
                <div className="flex items-center justify-between gap-2">
                  <dt className="flex items-center gap-1 text-muted-foreground"><Clock className="size-3.5" /> Next expected stage</dt>
            ...
            <GlassCard>
              <Label htmlFor="lodgement" className="text-sm font-medium">Lodgement Date</Label>
              <p className="mb-2 text-xs text-muted-foreground">Date the documents were, or are expected to be, lodged at the Deeds Office.</p>
              <Input id="lodgement" type="date" value={date} onChange={(e) => setDate(e.target.value)} />
              <Button size="lg" className="mt-4 w-full" onClick={submit}>Submit Update</Button>
            </GlassCard>
          </motion.div>
        )}
        ...
        {view === "success" && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            <GlassCard className="border-success/30 bg-success/5 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Thank you. The agency has been notified of the new lodgement date.</p>
            </GlassCard>
          </motion.div>
        )}
        ...
        {view === "expired" && (
          <GlassCard className="bg-muted/30 text-center text-muted-foreground">
            <p className="font-display text-lg font-semibold text-foreground">This link has expired.</p>
        ...
        {!loading && (
          <div className="mt-8 flex flex-wrap items-center gap-2 border-t border-dashed border-border pt-4 text-xs text-muted-foreground">
            <span>Demo:</span>
            <Button size="sm" variant="outline" onClick={() => setState("form")}>Form</Button>
            <Button size="sm" variant="outline" onClick={() => setState("success")}>Success</Button>
            <Button size="sm" variant="outline" onClick={() => setState("expired")}>Expired</Button>
          </div>
        )}
      </main>
    </div>
