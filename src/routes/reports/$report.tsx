...
const REPORT_META: Record<ReportKey, { title: string; description: string }> = {
  pipeline: {
    ...
    description: "Deals by stage, average time-in-stage, and stage conversion.",
  },
  "fall-through": {
    title: "Fall-Through Report",
    description: "Why deals cancel, and how the trend is moving.",
  },
  commission: {
    title: "Commission Report",
    description: "Agent earnings, cumulative payouts and agency totals.",
  },
  compliance: {
    title: "Compliance Report",
    description: "FFC currency and FICA completion across the agency.",
  },
};
...
export const Route = createFileRoute("/reports/$report")({
  ...
  head: ({ loaderData }) => {
    const meta = loaderData
      ? REPORT_META[loaderData.report]
      : { title: "Report", description: "Agency report." };
    return {
      meta: [
...
function PipelineReport() {
  ...
  const avgDaysInStage = useMemo(() => {
    return STAGES.map((stage) => {
      ...
      const avg = inStage.length
        ? Math.round(
            inStage.reduce(
              (a, d) => a + (Date.now() - new Date(d.stageSince).getTime()) / 86400000,
              0,
            ) / inStage.length,
          )
        : 0;
  ...
  const funnel = useMemo(() => {
    ...
    return STAGES.map((stage, i) => {
      const reached = deals.filter(
        (d) => STAGES.indexOf(d.stage) >= i || (d.cancelled && STAGES.indexOf(d.stage) >= i),
      ).length;
  ...
  const csvRows: (string | number)[][] = [
    ...
    ...STAGES.map((stage, i) => [
      stage,
      ...branches.map(
        (b) => activeDeals.filter((d) => d.stage === stage && d.branch === b.name).length,
      ),
            ...
            <BarChart data={byStageBranch} margin={{ left: 0, right: 12 }}>
              ...
              {branches.map((b, i) => (
                <Bar
                  ...
                  radius={i === branches.length - 1 ? [4, 4, 0, 0] : undefined}
                />
              ))}
            </BarChart>
          </ResponsiveContainer>
...
function FallThroughReport() {
  ...
  const trend = monthlyCommission.map((m, i) => ({
    ...
    cancellations: Math.max(
      1,
      Math.round((fallThroughReasons[i % fallThroughReasons.length].count + i) / 1.6),
    ),
  }));
        ...
        <div className="h-[340px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={byAgent}>
              ...
              <Tooltip
                formatter={(v: number) => zar(v, { decimals: false })}
                contentStyle={{
                  background: "var(--card)",
                  border: "1px solid var(--border)",
                  borderRadius: 8,
            ...
            {monthlyCommission.map((m, i) => (
              <TableRow key={m.month}>
                ...
                <TableCell className="money text-right">
                  {zar(m.gross, { decimals: false })}
                </TableCell>
                <TableCell className="money text-right">
                  {zar(m.agent, { decimals: false })}
                </TableCell>
                <TableCell className="money text-right">
                  {zar(cumulative[i].cumulative, { decimals: false })}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
function ComplianceReport() {
  ...
  const csvRows: (string | number)[][] = [
    ...
    ...ficaByPartyType.map((f) => [
      `FICA complete — ${f.type}`,
      `${f.complete}/${f.total} (${f.pctComplete}%)`,
    ]),
  ];
      ...
      <GlassCard>
        ...
        <div className="overflow-x-auto scrollbar-thin">
          <Table>
            ...
            <TableBody>
              {users.map((u) => {
                const days = u.ffc?.expiry
                  ? Math.round((new Date(u.ffc.expiry).getTime() - Date.now()) / 86400000)
                  : null;
                const status = !u.ffc
                  ? "No FFC"
                  : days === null
