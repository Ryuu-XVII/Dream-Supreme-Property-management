...
export const Route = createFileRoute("/commission/reconciliation")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Reconcile registered deals, practitioner payouts and clawbacks for each commission run." },
      { property: "og:title", content: "Monthly Commission Reconciliation | Dream Supreme Properties" },
      { property: "og:description", content: "Reconcile registered deals, practitioner payouts and clawbacks for each commission run." },
    ],
  }),
...
function ReconciliationPage() {
  ...
  const status: RunStatus = statusByPeriod[key] ?? (registeredDeals.length > 0 ? "Draft" : "Not Started");
  ...
  const practitionerRows = useMemo(() => {
    const map = new Map<string, { gross: number; desk: number; advance: number; clawback: number; net: number }>();
  ...
  const totals = useMemo(() => {
    ...
    const franchise = registeredDeals.reduce((s, dl) => {
      const net = grossCommission(dl) - Math.round(grossCommission(dl) - grossCommission(dl) / (1 + VAT_RATE));
      return s + Math.round((net * (rules.deductions.find((x) => x.type === "Franchise Fee")?.bps ?? 0)) / 10000);
    }, 0);
    const officeShare = registeredDeals.reduce((s, dl) => {
      const net = grossCommission(dl) - Math.round(grossCommission(dl) - grossCommission(dl) / (1 + VAT_RATE));
      const franchiseAmt = Math.round((net * (rules.deductions.find((x) => x.type === "Franchise Fee")?.bps ?? 0)) / 10000);
  ...
  const exportCsv = () => {
    ...
    const rows = registeredDeals.map((dl) => {
      ...
      return [dl.ref, `"${prop.address}, ${prop.suburb}"`, (dl.salePrice / 100).toFixed(2), (grossCommission(dl) / 100).toFixed(2), (netPayable(dl) / 100).toFixed(2), `"${agents}"`].join(",");
    });
  ...
  const statusTone =
    ...
    : status === "Draft" ? "border-warning/40 bg-warning/15 text-warning"
    ...
    >
      ...
      <div className="mb-6 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <Button variant="outline" size="icon" onClick={() => shiftPeriod(-1)}><ChevronLeft className="size-4" /></Button>
          <span className="font-display min-w-[140px] text-center text-lg font-semibold">{MONTHS[period.month]} {period.year}</span>
          <Button variant="outline" size="icon" onClick={() => shiftPeriod(1)}><ChevronRight className="size-4" /></Button>
        </div>
        <Badge variant="outline" className={statusTone}>{status}</Badge>
      </div>
        ...
        {can && status !== "Approved" && registeredDeals.length > 0 && (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button className="gap-2"><CheckCircle2 className="size-4" /> Approve Run</Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Approve commission run?</AlertDialogTitle>
                <AlertDialogDescription>
                  This will lock the {MONTHS[period.month]} {period.year} commission run for {registeredDeals.length} registered
                  deal(s) totalling {zar(totals.agentPayouts)} in agent payouts. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <GlassCard className="mb-6">
        ...
        {loading ? (
          ...
          <EmptyState title="No deals registered" message="No deals were registered in this period." />
        ) : (
                ...
                {registeredDeals.map((dl) => {
                  ...
                  return (
                    <TableRow key={dl.id}>
                      ...
                      <TableCell className="min-w-0 max-w-[220px] truncate">{prop.address}, {prop.suburb}</TableCell>
                      <TableCell className="money text-right whitespace-nowrap">{zar(dl.salePrice)}</TableCell>
                      <TableCell className="money text-right whitespace-nowrap">{zar(grossCommission(dl))}</TableCell>
                      <TableCell className="money text-right whitespace-nowrap">{zar(netPayable(dl))}</TableCell>
                      <TableCell>
                        <div className="flex -space-x-1.5">
                          {dl.practitioners.map((p) => <AgentAvatar key={p.userId} user={userById(p.userId)} />)}
                        </div>
                      </TableCell>
                    </TableRow>
                  );
      ...
      <GlassCard className="mb-6">
        ...
        {practitionerRows.length === 0 ? (
          <EmptyState title="No practitioner activity" message="No agent payouts to summarise for this period." />
        ) : (
            ...
            {practitionerRows.map((r) => (
              <div key={r.user.id} className="rounded-xl border border-border p-4">
                ...
                <dl className="space-y-1.5 text-sm">
                  <div className="flex justify-between"><dt className="text-muted-foreground">Gross allocation</dt><dd className="money">{zar(r.gross)}</dd></div>
                  <div className="flex justify-between"><dt className="text-muted-foreground">Desk fees</dt><dd className="money text-destructive">− {zar(r.desk)}</dd></div>
                  <div className="flex justify-between"><dt className="text-muted-foreground">Advance recoveries</dt><dd className="money text-destructive">− {zar(r.advance)}</dd></div>
                  {r.clawback > 0 && (
                    <div className="flex justify-between"><dt className="text-muted-foreground">Clawbacks</dt><dd className="money text-destructive">− {zar(r.clawback)}</dd></div>
                  )}
                </dl>
      ...
      {cancelledDeals.length > 0 && (
        <GlassCard className="mb-6 border-destructive/30">
          <h2 className="mb-4 font-display text-lg font-semibold text-destructive">Clawback Section — Cancelled Deals</h2>
          <div className="overflow-x-auto scrollbar-thin">
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {cancelledDeals.map((dl) => {
                  ...
                  return (
                    <TableRow key={dl.id}>
                      <TableCell className="money whitespace-nowrap">{dl.ref}</TableCell>
                      <TableCell className="min-w-0 max-w-[200px] truncate">{prop.address}</TableCell>
                      <TableCell>{dl.cancelled?.reason}</TableCell>
                      <TableCell className="whitespace-nowrap">{dl.cancelled ? dateFmt(dl.cancelled.at) : "—"}</TableCell>
                      <TableCell className="money text-right whitespace-nowrap text-destructive">− {zar(amt)}</TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
...
function TotalTile({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className={`rounded-lg border p-3 ${highlight ? "border-primary/30 bg-primary/5" : "border-border"}`}>
      ...
      <p className={`money mt-1 text-lg font-bold ${highlight ? "text-primary" : ""}`}>{zar(value)}</p>
    </div>
  );
}
